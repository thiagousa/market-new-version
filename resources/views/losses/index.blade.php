        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Add New', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('lossesAddStepOne').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $loss->lossesId }}</td>
                            <td class="font-w600">{{ (($loss->product->shortName) ? $loss->product->shortName : $loss->product->description) }}</td>
                            <td class="font-w600">{{ $loss->product->category->name }}</td>
                            <td class="font-w600">{{ $loss->type }}</td>
                            <td class="font-w600">{{ $loss->updated_at->format('m/d/Y') }}</td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-file"></i> ', ['title'=>'View', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn btn-info',
                                    'onclick'=>'window.open(\''.route('lossesView', $loss->lossesId).'\', \'_self\')']) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                        @endforeach
