        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Add New', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('categoriesAdd').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $category->categoriesId }}</td>
                            <td class="font-w600">{{ $category->name }}</td>
                            <td class="font-w600">{{ $category->tax.' %' }}</td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-pencil"></i> ', ['title'=>'Edit', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-success',
                                    'onclick'=>'window.open(\''.route('categoriesEdit', $category->categoriesId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'formDelete'.$category->categoriesId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('categoriesId', $category->categoriesId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Delete', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('categoriesDelete'), 'data-form'=>1, 'data-id-form'=>'formDelete'.$category->categoriesId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                        @endforeach
