        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Add New', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('typeCustomersAdd').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $typeCustomer->typeCustomersId }}</td>
                            <td class="font-w600">{{ $typeCustomer->name }}</td>
                            <td class="font-w600">{{ $typeCustomer->tax.' %' }}</td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-pencil"></i> ', ['title'=>'Edit', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-success',
                                    'onclick'=>'window.open(\''.route('typeCustomersEdit', $typeCustomer->typeCustomersId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'formDelete'.$typeCustomer->typeCustomersId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('typeCustomersId', $typeCustomer->typeCustomersId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Delete', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('typeCustomersDelete'), 'data-form'=>1, 'data-id-form'=>'formDelete'.$typeCustomer->typeCustomersId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                        @endforeach
