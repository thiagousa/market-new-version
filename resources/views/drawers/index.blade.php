        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Add New', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('drawersAdd').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $drawer->number }}</td>
                            <td class="font-w600">$ {{ $drawer->moneyFormatted()}}</td>
                            <td class="font-w600">{{ $drawer->status }}</td>
                            <td class="font-w600">{{ $drawer->updated_at->format('m/d/Y') }}</td>
                            <td class="text-center">
                                @if (!$drawer->status)
                                {!! Form::button('<i class="fa fa-pencil"></i> ', ['title'=>'Edit', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-success',
                                    'onclick'=>'window.open(\''.route('drawersEdit', $drawer->drawerId).'\', \'_self\')']) !!}


                                {!! Form::open([
                                    'id' => 'formDelete'.$drawer->drawersId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('drawersId', $drawer->drawersId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Delete', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('drawersDelete'), 'data-form'=>1, 'data-id-form'=>'formDelete'.$drawer->drawerId]) !!}
